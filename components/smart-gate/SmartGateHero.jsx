      ...
      <div className="wrapper hero-grid">
        <div>
          ...
          <h1 className="text-[clamp(2.6rem,6vw,5.1rem)] leading-[0.95] tracking-tight m-[24px_0_22px] max-w-[12ch] font-extrabold text-[#071A33]">
            Build a gate that <span className="text-[var(--orange)]">detects.</span>
          </h1>
          <p className="text-[1.12rem] text-[#16324F] max-w-[62ch]">
            Turn an everyday problem into a working invention. You will give an automatic boom gate “eyes” to detect a vehicle, a “brain” to make decisions, and a “muscle” to lift the barrier.
          </p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="/smart-gate-lab" className="border-0 rounded-[14px] p-[14px_18px] font-black inline-flex items-center gap-[9px] no-underline transition-transform hover:-translate-y-0.5 bg-gradient-to-br from-[#00a8c5] to-[#087d94] text-white shadow-[0_14px_30px_rgba(0,184,217,.2)]">
              ⚡ Enter Live Circuit Lab
            </Link>
            <a href="/downloads/smart_gate.ino" download className="border border-[var(--line)] rounded-[14px] p-[14px_18px] font-black inline-flex items-center gap-[9px] no-underline transition-transform hover:-translate-y-0.5 bg-white text-[var(--ink)]">
        ...
        <div className="relative bg-gradient-to-br from-[#0A2547] to-[#071A33] rounded-[34px] p-[28px] text-white shadow-[0_30px_80px_rgba(7,26,51,.28)] overflow-hidden min-h-[500px]">
          ...
          <div className="relative z-10 h-[360px] grid place-items-center mt-3">
            <div className={`gate-illustration ${demoOpen ? 'open' : ''}`} onClick={() => setDemoOpen(!demoOpen)}>
              <div className="gate-signal"><span></span><span></span><span></span></div>
              <div className="vehicle"></div>
              <div className="gate-boom"></div>
              <div className="gate-post"><div className="gate-sensor"><span></span><span></span></div><div className="gate-led"></div></div>
            </div>
          </div>
          ...
          <div className="relative z-10 flex justify-center -mt-2.5">
            <button type="button" onClick={() => setDemoOpen(!demoOpen)} className="border border-white/20 bg-white/10 text-white rounded-full p-[11px_16px] font-black cursor-pointer hover:bg-white/20">
              {demoOpen ? 'Move vehicle away' : 'Bring vehicle closer'}
            </button>
          </div>
        </div>
      </div>
