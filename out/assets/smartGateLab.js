(() => {
  ...
  const STORAGE = 'maskido-smart-gate-lab-v1';
  const COURSE_KEY = 'maskido-smart-gate-progress-v1';
  ...
  const defaultPositions = {
    arduino: {x: 75, y: 220}, sensor: {x: 430, y: 80}, servo: {x: 615, y: 355}, power: {x: 390, y: 460}, breadboard: {x: 365, y: 260}, led: {x: 400, y: 350}, buzzer: {x: 500, y: 350}
  };

  const partSizes = {
    arduino: [260,176], sensor:[190,105], servo:[130,102], power:[140,88], breadboard:[260,125], led:[40,40], buzzer:[40,40]
  };

  const requiredParts = ['arduino','sensor','servo','power','led','buzzer'];
  const requiredEdges = [
    edgeKey('arduino','5V','sensor','VCC'),
    edgeKey('arduino','GND','sensor','GND'),
    edgeKey('arduino','D9','sensor','TRIG'),
    edgeKey('arduino','D10','sensor','ECHO'),
    edgeKey('arduino','D3','servo','SIG'),
    edgeKey('power','5V','servo','VCC'),
    edgeKey('power','GND','servo','GND'),
    edgeKey('arduino','GND2','power','GND2'),
    edgeKey('arduino','D5','led','RED'),
    edgeKey('arduino','D6','led','GREEN'),
    edgeKey('arduino','GND','led','GND'),
    edgeKey('arduino','D7','buzzer','PLUS'),
    edgeKey('arduino','GND','buzzer','GND')
  ];
  ...
  const pinDefs = {
    arduino: [
      ...
      {id:'D10',label:'D10',type:'signal',x:88,y:17},
      {id:'D9',label:'D9',type:'signal',x:112,y:17},
      {id:'D7',label:'D7',type:'signal',x:136,y:17},
      {id:'D6',label:'D6',type:'signal',x:160,y:17},
      {id:'D5',label:'D5',type:'signal',x:184,y:17},
      {id:'D3',label:'D3',type:'signal',x:208,y:17}
    ],
    sensor: [
    ...
    led: [
      {id:'RED',label:'R',type:'signal',x:10,y:0},
      {id:'GREEN',label:'G',type:'signal',x:30,y:0},
      {id:'GND',label:'-',type:'ground',x:20,y:40}
    ],
    buzzer: [
      {id:'PLUS',label:'+',type:'signal',x:10,y:0},
      {id:'GND',label:'-',type:'ground',x:30,y:0}
    ],
    breadboard: []
  };
  ...
  function componentMarkup(type){
    ...
    if(type === 'led') return `<div style="width:40px;height:40px;border-radius:50%;background:#FF4D4D;border:3px solid #B30000;"></div>`;
    if(type === 'buzzer') return `<div style="width:40px;height:40px;border-radius:50%;background:#333;border:3px solid #111;"></div>`;
    return `<div class="breadboard"></div>`;
  }

  function partLabel(type){
    return ({arduino:'Arduino Uno · brain',sensor:'HC-SR04 · eyes',servo:'SG90 Servo · muscle',power:'Safe 5V · energy',breadboard:'Mini breadboard · optional',led:'Red/Green LED · indicator',buzzer:'Buzzer · alarm'})[type];
  }
  ...
  function startSimulation(){
    ...
    $('#simGate').classList.add('powered');updateLiveSimulation();addCourseProgress(6);if(!storeGet('sessionStorage','simBonus')){storeSet('sessionStorage','simBonus','1');addXP(100,'Digital twin activated');}window.scrollTo({top:0,behavior:'smooth'});loopAnimation();
  }
  ...
  function updateLiveSimulation(){
    ...
    const handRight=-155+Math.round((45-d)/43*210);$('#virtualVehicle').style.right=`${handRight}px`;
    if(d<=12) state.lidOpen=true; else if(d>18) state.lidOpen=false;
    $('#simGate').classList.toggle('open',state.lidOpen);const angle=state.lidOpen?90:0;$('#servoAngle').textContent=angle;
    $('#decisionText').textContent=state.lidOpen?'OPEN':'WAIT';$('#decisionState').textContent=state.lidOpen?'VEHICLE DETECTED':'VEHICLE IS FAR';$('#servoState').textContent=state.lidOpen?'OPEN':'CLOSED';$('#sensorState').textContent=d<=12?'OBJECT FOUND':'SCANNING';
    $('#consoleText').textContent=`Distance: ${d} cm\nDecision: ${state.lidOpen?'OPEN GATE':'KEEP CLOSED'}\nServo target: ${angle}°`;
  ...
  const rounds=[
    {title:'Spot the eyes',subtitle:'Which component measures the distance to a vehicle?',type:'component',answer:'sensor'},
    ...
    {title:'Predict the gate',subtitle:'A vehicle is 8 cm away. What should the gate do?',type:'predict',answer:'open'},
  ...
  function renderRound(){
    ...
    if(r.type==='predict') body=`<div class="visual-options"><button class="visual-option" data-answer="closed" type="button"><span class="prediction-visual"><span class="prediction-gate"></span><span class="prediction-vehicle">🚙</span></span><strong>Stay closed</strong></button><button class="visual-option" data-answer="open" type="button"><span class="prediction-visual"><span class="prediction-gate" style="--boom-angle:-75deg"></span><span class="prediction-vehicle" style="--vehicle-x:-20px">🚙</span></span><strong>Open the gate</strong></button><button class="visual-option" data-answer="reset" type="button"><span style="font-size:3rem">↻</span><strong>Restart Arduino</strong></button></div>`;
  ...
  function feedbackFor(round,correct){
    ...
    const bad=['Look for the component with two round ultrasonic transducers.','TRIG is connected to Arduino D10; ECHO returns on D9.','Any distance at or below 12 cm opens the gate.','5V → GND is the dangerous fault. The other two are correct signal/reference paths.','The correct flow is Sense → Decide → Act.'];return correct?good[round]:bad[round];
  }
